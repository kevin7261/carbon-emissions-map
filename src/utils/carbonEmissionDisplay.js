...
import { wrapFormattedNumberStringWithFractionXsHtml } from '@/utils/numberDisplay.js';
...
/**
 * 數字（維持原 CSV 字串；小數段 &lt;small&gt;）＋ 不換行空格 ＋ 公噸CO&lt;sub&gt;2&lt;/sub&gt;e
 */
export function formatCarbonEmissionQuantityHtml(value) {
  const raw = value == null ? '' : String(value).trim();
  if (raw === '') return '';
  return `${wrapFormattedNumberStringWithFractionXsHtml(raw)}\u00A0<span class="carbon-emission-unit">公噸CO<sub>2</sub>e</span>`;
}
